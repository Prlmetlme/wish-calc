...
const Form = () => {
  ...
  const [currentPreset, setCurrentPreset] = useState(
    typeof window !== "undefined" && localStorage.getItem("currentPreset")
      ? localStorage.getItem("currentPreset")!
      : "preset1"
  );
  ...
  const reducer = (state: Preset, action: DispatchActions) => {
    switch (action.type) {
      ...
      case "updateAdvancedSettings":
        return {
          ...state,
          advancedSettings: action.payload as AdvancedSettings,
        };
      case "updateAll":
  ...
  useEffect(() => {
    dispatch({
      type: "updateAll",
      payload: JSON.parse(localStorage.getItem(currentPreset)!)
        ? JSON.parse(localStorage.getItem(currentPreset)!)
        : {
            fates: 0,
            primos: 0,
            starglitter: 0,
