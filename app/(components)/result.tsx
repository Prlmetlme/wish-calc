...
const Result = (props: {
  ...
  if (props.state) {
    ...
    const wishes =
      fates + Math.floor(primos / 160) + Math.floor(starglitter / 5);
        ...
        if (FIRST_RUN) {
          if (private_guaranteed) {
            if (private_wishes + pity >= 90) {
              ...
              probabilities.push(
                1 - BASE_PROBABILITY ** private_wishes + soft_pity_modifier
              );
            ...
            if (private_wishes + pity >= 180) {
              ...
              probabilities.push(
                1 - BASE_PROBABILITY ** (private_wishes - 90) + 0.5
              );
            } else {
          ...
          if (probabilities[i - 1] === 1) {
            ...
            ); // Calculate the amount of wishes from the last character
    ...
    const probabilities = calculate(
      wishes,
      pity,
      isGuaranteed,
      advancedSettings.amountOfCharactersDesired
    );

    const openAdvancedSettings = (e: React.MouseEvent<HTMLButtonElement>) => {
      props.dispatch({
        type: "updateUseAdvancedSettings",
        payload: !useAdvancedSettings,
      });
    ...
    return (
      <div className="container scara-bg">
        <p className="wishes" style={{ color: colorCalc(wishes) }}>
          {wishes}
        </p>
        <p>
          Guaranteed <span style={{ color: "#eaad00" }}>5★</span>:{" "}
          {wishCalc("guaranteed 5*", wishes)}
        </p>

        {wishCalc("guaranteed 5*", wishes) !==
        wishCalc("average 5*", wishes) ? (
          <p>
            Average<span style={{ color: "#eaad00" }}> 5★</span>:{" "}
            {wishCalc("average 5*", wishes)}
          </p>
        ) : null}

        <p>
          Guaranteed <span style={{ color: "#b880dd" }}>4★</span>:{" "}
          {wishCalc("guaranteed 4*", wishes)}
        </p>
        <p>
          Average <span style={{ color: "#b880dd" }}>4★</span>:{" "}
          {wishCalc("average 4*", wishes)}
        </p>
        ...
        {useAdvancedSettings ? (
          <div className="advanced-results">
            ...
            <p>
              Guaranteed <span style={{ color: "#eaad00" }}>5★</span> in x days:
            </p>
            <p>
              Average <span style={{ color: "#eaad00" }}>5★</span> in x days:
            </p>
            <p>
              Guaranteed <span style={{ color: "#b880dd" }}>4★</span> in x days:
            </p>
            <p>
              Average <span style={{ color: "#b880dd" }}>4★</span> in x days:
            </p>
