export const verifyInputIsNumber = (
...
export const generalizeProbability = (
  probability: number
): { probability: string; color: string; estimate: string } => {
  ...
  switch (true) {
    ...
    case roundedProbability > 50 && roundedProbability <= 79.99:
      return {
        probability: "Likely",
        color: "green",
        estimate: "between 50% and 80%",
      };
    case roundedProbability >= 80 && roundedProbability < 100:
      return {
        probability: "Very likely",
        color: "#d6851e",
        estimate: "above 80%, but less than 100%",
      };
...
export const handleKeyboardEvent = (
  ...
  const elements = Array.from(e.currentTarget.parentElement!.children).filter(
    (element) => {
...
export interface DispatchActions {
  type:
    ...
    | "updatePity"
    | "updateIsGuaranteed"
    | "updateUseAdvancedSettings"
    | "updateAdvancedSettings"
    | "updateDaysPlanned"
    | "updateWelkinDays"
    | "updateCharacterPlanning"
    | "updateAll";
